...
async function verificaProdutos() {
    ...
    try {
        ...
        for (i = 0; i <= produtos.length - 1; i++) {
            ...
            let dadosproduto        = await verificaNome(produtos[i].Codigo_produto);
            console.log(dadosproduto.tipo_item)
            if (dadosproduto.tipo_item == '001' || dadosproduto.tipo_item == '005') {
                ...
                const gravaanalise = await iql.necessidade.create({
                    data: {
                        ...
                        desc_tecnica  : dadosproduto.descricao,
...
async function verificaNome(Codigo_produto) {
    ...
    const nome = await corp.$queryRaw`
    select ia.desc_compra as Descricao,  ia.Codigo_tipo_item
    from [dbo].[Itens_almox] AS ia 
    where 
        ia.Apelido_empresa = 'IQL Tintas'
    AND ia.Apelido_estabelecimento = 'Matriz'
    AND ia.Codigo_produto = ${Codigo_produto}
    `;
    ...
    if (nome.length != 0) {
        ...
        descricao = nome[0].Descricao;
        tipo_item = nome[0].Codigo_tipo_item

    }
    console.log("descrição : " + descricao)
    return { descricao, tipo_item };
}
...
async function verificaInventario(Codigo_produto, dtfinal) {
    ...
    where hi.cod_produto = ${Codigo_produto} and
    hi.data <= ${dtfinal}
    group by 
    hi.cod_produto,
    hi.data
    ...
    if (inventario.length != 0) {
        console.log('Inventario' + inventario[0]);
  ...
  ir.[Id_caparec]              = cr.[Id_caparec]
    ...
    `
        ...
        for (w = 0; w <= fornecedors.length - 1; w++) {
            ...
            SELECT  
              tt.nrtitulo, 
              ISNULL(tt.Data_Emissao, '1900-01-01') AS Data_Emissao, 
              ISNULL(tt.data_vencto, '1900-01-01') AS data_vencto, 
              ISNULL(tt.data_pagto, '1900-01-01') AS data_pagto, 
              tt.Valor_titulo, 
              tt.Saldo_titulo,
