...
async function verificaProdutos() {
    ...
    try {
        ...
        for (i = 0; i <= produtos.length - 1; i++) {
            ...
            let opfinalizada = await verificaFinalizadoIntermediario(produtos[i].Codigo_produto, datainventario);

            let opemprocesso = await verificaEmprocessoIntermediario(produtos[i].Codigo_produto, datainventario);

            let opcadastrada15 = await verificaCadastrada15Intermediario(produtos[i].Codigo_produto, dtfinal);

            let ultimacoleta15 = await verificaColeta15Intermediario(produtos[i].Codigo_produto, dtfinal);
            let opcompra15 = await verificaCompra15Intermediario(produtos[i].Codigo_produto, dtfinal);
            let opcompraparc = await verificaCompraParcialItermediario(produtos[i].Codigo_produto, datainventario);
            let opcompraEntregue = await verificaCompraEntregueIntermediario(produtos[i].Codigo_produto, datainventario);
...
async function verificaCompra15Intermediario(Codigo_produto, dtfinal) {

    opcompra15 = 0;

    const compra = await corp.$queryRaw`
      select SUM(io3.Quantidade_inicial) as opcompra
    from [dbo].[Itens_op] as io3
    LEFT JOIN [dbo].[Capa_op] AS co3 ON 
    (   co3.Apelido_empresa = io3.Apelido_empresa AND
       co3.Apelido_estabelecimento = io3.Apelido_estabelecimento AND
    ...
    where 
        io3.Apelido_empresa         = 'IQL Tintas' 
    and io3.Apelido_estabelecimento = 'Matriz'
    and co3.Status_op = 'Cadastro' 
    and co3.Data_inicio >=  ${dtfinal}
    and co3.Codigo_produto = ${Codigo_produto}
    group by
    io3.Apelido_empresa ,       
    io3.Apelido_estabelecimento ,
    co3.Codigo_produto
...
async function verificaCompraParcialItermediario(Codigo_produto, datainventario) {

    opcompraparc = 0;

    const compra = await corp.$queryRaw`

        select SUM(io3.Quantidade_inicial) as opcompraparc
    from [dbo].[Itens_op] as io3
    LEFT JOIN [dbo].[Capa_op] AS co3 ON 
    (   co3.Apelido_empresa = io3.Apelido_empresa AND
       co3.Apelido_estabelecimento = io3.Apelido_estabelecimento AND
...
async function verificaCompraEntregueIntermediario(Codigo_produto, datainventario) {

    opcompraEntregue = 0;

    const compraatendido = await corp.$queryRaw`
        select SUM(io.Quantidade_final) as opcompraEntregue
    from [dbo].[Itens_op] as io
    LEFT JOIN [dbo].[Capa_op] AS co ON 
   (   co.Apelido_empresa = io.Apelido_empresa AND
       co.Apelido_estabelecimento = io.Apelido_estabelecimento AND
...
//            destinatario: "luis.espoladore@adcon.srv.br;samuel.corazza@adcon.srv.br",
            destinatario: "luis.espoladore@adcon.srv.br;samuel.corazza@adcon.srv.br;alexandre.golveia@iql.com.br;elaine.dutra@iql.com.br",
...
async function geraemailResumo(relcompras) {
    ...
    this.html = this.html + `
    <h2 style="color: #0000cd;">Resumo Necessidade Intermediario: </h2>
    <p>&nbsp;</p>
    <table style="border-collapse: collapse;" border="1">
    <tbody>
    <tr>
    ...
    <td style="text-align: center"><strong>Produto</strong></td>
    <td style="text-align: center;font-family: Arial, sans-serif;"><strong>Descrição</strong></td>
    <td style="text-align: center"><strong>Fabricar</strong></td>
    <td style="text-align: center"><strong>Data Geração</strong></td>  
    <td style="text-align: center"><strong>Cobre Necessidade Até</strong></td>  
 ...
 destinatario: "luis.espoladore@adcon.srv.br;samuel.corazza@adcon.srv.br;alexandre.golveia@iql.com.br;elaine.dutra@iql.com.br",
            ...
            //destinatario: "luis.espoladore@adcon.srv.br;samuel.corazza@adcon.srv.br;andreia.oliveira@iql.com.br",
