...
--        tt.Valor_titulo  AS Valor,
    ...
    UNION ALL
SELECT 
    CAST(mt.Data_integracao AS DATE) AS Data,
    '0' AS Codigo_Centro_Custo,
    '(+) Recebimentos' AS Descricao_Centro_Custo1,
    '0' AS Codigo_Aplicacao_Compras,
    '(+) Recebimento de Clientes' AS Descricao_Aplicacao_Compras1,
    mt.Valor AS Valor,
    tt.Valor_titulo AS Valor_titulo,
    tt.Saldo_titulo AS Saldo_titulo
FROM dbo.Titulos tt
LEFT JOIN dbo.movto_titulo AS mt 
    ON tt.Apelido_empresa = mt.Apelido_empresa
    AND tt.Apelido_estabelecimento = mt.Apelido_estabelecimento
    AND tt.id_titulo = mt.id_titulo
LEFT JOIN dbo.Agente AS ag 
    ON tt.Apelido_empresa = ag.Apelido_empresa 
    AND tt.Apelido_estabelecimento = ag.Apelido_estabelecimento 
    AND tt.Codigo_agentes = ag.Codigo_agentes
WHERE 
    tt.pag_rec = 'R'
    AND MONTH(mt.Data_integracao) = ${mes}  -- Mês de fevereiro
    AND YEAR(mt.Data_integracao) = ${ano}  -- Ano 2025
    AND tt.cancelado = 0
    AND tt.previsao = 0
    AND mt.Codigo = 1012
    AND NOT EXISTS (
        SELECT 1 
        FROM dbo.movto_titulo mt2
        WHERE mt2.Apelido_empresa = tt.Apelido_empresa
        AND mt2.Apelido_estabelecimento = tt.Apelido_estabelecimento
        AND mt2.id_titulo = tt.id_titulo
        AND mt2.Codigo = 32
    )
    ...
    union all
        ...
        CAST(mt.Data_integracao AS DATE) AS Data,
        '0' AS Codigo_Centro_Custo,
        '(-) Despesas Financeiras'  AS Descricao_Centro_Custo1,
        '0' AS Codigo_Aplicacao_Compras1,
        '(-) Juros de Antecipações' AS Descricao_Aplicacao_Compras1,
        mt.Valor  AS Valor,
        tt.Valor_titulo AS Valor_titulo,
        tt.Saldo_titulo AS Saldo_titulo
    FROM dbo.Titulos tt
    LEFT JOIN [dbo].movto_titulo AS mt ON
            tt.Apelido_empresa = mt.Apelido_empresa
        AND tt.Apelido_estabelecimento = mt.Apelido_estabelecimento
        AND tt.id_titulo = mt.id_titulo
	LEFT JOIN dbo.[Agente] AS ag ON 
            tt.Apelido_empresa = ag.Apelido_empresa 
        AND tt.Apelido_estabelecimento = ag.Apelido_estabelecimento 
        AND tt.[Codigo_agentes] = ag.[Codigo_agentes]
    WHERE 
            tt.pag_rec = 'R'
...
union all
    ...
    SELECT 
        ...
        '(+) Recebimentos'  AS Descricao_Centro_Custo1,
        '0' AS Codigo_Aplicacao_Compras1,
        '(+) Fomento' AS Descricao_Aplicacao_Compras1,
        mt.Valor  AS Valor,
        tt.Valor_titulo AS Valor_titulo,
        tt.Saldo_titulo AS Saldo_titulo
    FROM dbo.Titulos tt
    ...
    WHERE 
            tt.pag_rec = 'R'
        AND MONTH(mt.Data_integracao) =  ${mes} --MONTH(GETDATE())
        AND YEAR(mt.Data_integracao) = ${ano}  --YEAR(GETDATE())
		AND tt.cancelado =0
		AND tt.previsao  = 0
		AND mt.Codigo = 32
    ...
    FROM
        ...
        AND mt.Codigo IN ('1001','1012')
            `;
                ...
                FROM titulos tt
                LEFT JOIN [Agente] a 
                    ON tt.Apelido_empresa = a.Apelido_empresa
                    AND tt.Apelido_estabelecimento = a.Apelido_estabelecimento
                    AND tt.Codigo_Agentes = a.Codigo_Agentes
                LEFT JOIN agente_funcao af 
                    ON tt.Apelido_empresa = af.Apelido_empresa
                    AND tt.Apelido_estabelecimento = af.Apelido_estabelecimento
                    AND tt.Codigo_Agentes = af.Codigo_Agentes
                LEFT JOIN [Agente] a2 
                    ON af.Apelido_empresa = a2.Apelido_empresa
                    AND af.Apelido_estabelecimento = a2.Apelido_estabelecimento
                    AND af.Relacao_Agentes = a2.Codigo_Agentes
                LEFT JOIN movto_titulo mt 
                    ON mt.Apelido_empresa = tt.Apelido_empresa
                    AND mt.Apelido_estabelecimento = tt.Apelido_estabelecimento    
                    AND mt.nrtitulo = tt.nrtitulo
                LEFT JOIN Transacao_contas_receber AS tcr 
                    ON mt.Apelido_empresa = tcr.Apelido_empresa
                    AND mt.Apelido_estabelecimento = tcr.Apelido_estabelecimento
                    AND mt.Codigo = tcr.Codigo
                WHERE 
                    tt.data_vencto BETWEEN ${dtinicio} AND ${dtfinal}
                    AND tt.Apelido_empresa = 'iql tintas'
                    AND tt.pag_rec = 'r'
                    AND a.Nome NOT LIKE '%idea%'
                    AND a.Nome NOT LIKE '%acmc%'
                    AND af.Descricao_comercial IN ('vendedor', 'supervisor', 'gerente comercial')
                    AND (mt.nrtitulo IS NULL OR mt.Codigo NOT IN ('1001', '1012'))
                GROUP BY 
                    tt.Nro_NF, tt.Valor_Total_NF, tt.Valor_IPI, tt.nrtitulo, 
                    tt.valor_titulo, tt.saldo_titulo, tt.data_vencto, 
                    a.Nome, a2.Nome, af.Descricao_comercial, af.Comissao;
                                `;                
            }
        } else {
            if (pago) {
